...
(`control.bat` is the dev path that builds beside the repo; the released app is **standalone** — on first
...
- **Code:** Crush v0.76, exercised by a 15-task golden eval suite (`docs/scorecards/`). Claw Code bake-off'd → rejected (45%, flaky tool calls).
...
- **Control panel:** native WPF cockpit (`doki panel` / `DokiDex.lnk`) — polls status in-process (native C#, ~2 s) and shells `doki.ps1` only for lifecycle actions (start/stop/verify/gen); grouped live cards, GPU trust-meter, mode switcher with 32 GB-headroom + eviction confirm, live logs, per-modality ⚡test, coder model-swap, update badges. A **cinematic boot sequence** ("THE SEAL IGNITES" — gold FF summoning hexagram = Iron Man reactor → Star Trek LCARS rail booted from real `doki status json`) and an **app-wide premium re-theme** (deep void · one emitting cyan accent · gold as etched structure), both designed + adversarially reviewed via multi-agent workflows. **In-app auto-updater** (self-contained single-file exe on `v*` tags; safe in-place rename-and-relaunch swap). **124 unit tests** on its data, control-plane, and installer layers (`dotnet test control\DokiDex.Control.Tests`).
