...
function Resolve-GenKind {
    param([switch]$Video, [switch]$Music, [switch]$Edit)
    $picked = @()
    if ($Video) { $picked += 'video' }
    if ($Music) { $picked += 'music' }
    if ($Edit)  { $picked += 'edit'  }
    if ($picked.Count -gt 1) { throw "pick ONE of -Video / -Music / -Edit (got: $($picked -join ', '))" }
...
function Get-GenRecipe {
    param(
        [ValidateSet('image', 'video', 'music', 'edit')][string]$Kind = 'image',
