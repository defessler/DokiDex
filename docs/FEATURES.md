...
| Feature | What it does | How to use |
|---|---|---|
| **`doki verify`** — full-stack smoke test | Cycles GPU modes (agent → coexist → media), runs live capability smokes with a real API call each, prints a PASS/SKIP/FAIL table, restores `agent` (the default resting state), and exits 0 only if zero failures. **16 result rows: 5 always run, 11 SKIP cleanly when a full-tier model (or coder-big) is absent.** | `.\doki.ps1 verify` |
