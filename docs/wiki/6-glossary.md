...
**Agent**
An AI that doesn't just chat — it *does things*. It reads files, edits code, runs commands, and loops until the job's done. DokiCode is an agent.

**Harness**
The "body" around the brain — the program that gives it eyes and hands. It shows the brain your files, makes the edits the brain decides on, runs commands, and handles permissions. DokiCode's harness is **Crush**.

**Local vs. cloud**
*Local* = runs on your own computer; nothing leaves. *Cloud* = runs in a company's data center; your data is sent there. DokiCode is local (except optional plain web search).
...
**VRAM (the "desk space")**
The super‑fast memory *on the graphics card* — 32 GB here. A brain has to fit in VRAM to run fast. This "what fits on the desk" limit is the single biggest constraint in DokiCode.
...
**GGUF**
Just a file format for a downloadable brain — the kind of file you grab and run locally. If a model is "available as a GGUF," DokiCode can use it.
...
A brain design where, for each question, only the few relevant "experts" inside wake up instead of the whole thing. It's how DokiCode runs a 120‑billion‑knob brain without needing a data center — most of it stays asleep at any moment, so it's cheaper to run than its size suggests.

**Context window**
How much the brain can "hold in its head" at once — the current conversation plus the files it's looking at. Measured in *tokens*. DokiCode's is large (~128k tokens), but it still fills up on long sessions, which is why the advice is "one task per chat."
...
The special skill behind autocomplete: given the code *before* and *after* your cursor, guess what goes in the gap. DokiCode uses a tiny FIM model so it can finish your lines instantly as you type.
...
**llama-swap**
The *receptionist* — sits in front of the engine with one phone number and swaps which brain is loaded when you ask for a different one. Lets DokiCode juggle three brains through a single connection.
...
A *universal adapter* for plugging outside tools into an AI assistant. DokiCode uses it to plug in exactly one tool: web search.
...
**Crush**
DokiCode's hands (the harness) — the daily driver. A Windows‑friendly program that connects the brain to your files, terminal, and permissions. Won the bake‑off.

**OpenCode**
The runner‑up harness, kept around as a challenger to Crush.

**DuckDuckGo MCP**
The web‑search tool, plugged in via MCP. Keyless (no signup), and it's plain search — not an AI cloud service. The only thing in DokiCode that touches the internet.
...
The robot's "driving test" — a fixed set of real coding jobs, each with an automatic pass/fail check. Re‑run after any change to prove things got better, not worse. DokiCode's score: 10/11 (91%).
...
**Bake‑off**
A head‑to‑head contest to pick a tool by measured results instead of opinion. DokiCode used one to choose Crush over OpenCode.

**Git**
A system that snapshots your code so you can rewind to any earlier version. DokiCode's safety rule: only ever let the robot loose on code that's saved in Git — it's the undo button.
