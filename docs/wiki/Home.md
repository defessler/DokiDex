...
## DokiCode in one sentence

**DokiCode is a robot coding assistant that lives entirely inside one personal computer — it reads your code, writes code, runs commands, and looks things up on the web, all without ever sending your code to a company's cloud.**
...
| 4 | [Local vs. the Big Clouds](4-local-vs-the-big-clouds.md) | The honest comparison: where DokiCode wins, where Claude Code and Codex win, and why. |
...
That trade — *a bit less brilliant, but private, free, and entirely yours* — is the whole point of DokiCode.
