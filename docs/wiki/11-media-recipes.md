...
| **Fast video** (LTXV) | `model=ltxv-2b-0.9.8-distilled.safetensors, textvideoframes=97, steps=8, cfgscale=1, width=768, height=512, videofps=24, sampler=euler, scheduler=normal` (T5 auto-downloads first run; euler/normal = the LTXV 0.9.8 distilled template — without them the arm inherits SwarmUI's uni_pc/simple video defaults; do NOT raise CFG above 1) |
