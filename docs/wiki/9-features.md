...
| **coder-fast** — Qwen3-Coder-30B-A3B | Daily-driver coder MoE fully on GPU; 30B-A3B (UD-Q4_K_XL) @ 131072 ctx, ~26 GB VRAM, **265 tok/s**. | `model: coder-fast`. The **deployed** default for both Crush roles (`large` and `small`) is actually `coder-fast-lite` (65536 ctx) so the agent fits alongside FIM autocomplete in `coexist` without OOM; agent-only users can bump both roles to full-128k `coder-fast` by editing `harness/crush.json`. |
...
| **Crush coder CLI** | Daily-driver agentic coding CLI (bake-off winner, v0.76) wired to llama-swap with native tool calls. | Launch `crush` yourself. Config deployed to `~/.config/crush/crush.json` from `harness/crush.json` (provider `local`, base_url `:8080/v1/`, key `dummy`). |
| **`doki code` local coding agent** | Terminal coding agent that mirrors Claude Code: a ReAct loop over **Read** (line-windowed) / **Grep** (regex + optional path/glob) / **Edit** (SEARCH/REPLACE blocks, whitespace-flexible fuzzy applier, miss → nearby-line hint) / **Write** / **Bash** (PowerShell), plus opt-in **WebSearch**/**MemoryRecall** (`/tools web on`, off by default). Read/Grep/WebSearch/MemoryRecall run freely; Edit/Write/Bash are gated behind a per-action `[y]es / [a]lways / [n]o` approval (default: no) with a colored diff or command preview — `[a]lways` persists an allow/deny **permission rule** (`Tool` \| `Tool(specifier)`, `prefix *` matching, deny-first) to `%USERPROFILE%\.doki\permissions` instead of a session-only bypass. Content streams live by default (Esc/Ctrl+C interrupts). Auto-loads repo orientation (DOKI.md/AGENTS.md/CLAUDE.md + a depth-2 tree + git status, as a fixed system message; `/init` writes one). A context meter (`~Nk / 32k ctx`) plus `/compact` / auto-compact at ~40k / `/context` manage the working set. Sessions persist to `%USERPROFILE%\.doki\sessions` (`--continue`, `/resume`, `/export`). `/plan` mode explores read-only before `/act` applies changes; `.doki/commands/*.md` define custom `/name` commands; `@rel/path` mentions and `!command` shell passthrough work inline. In-session `/undo` stack; edits are plain working-tree changes (review with `/diff` or `git diff`). Up to 24 tool hops per turn. Default model: `coder-fast`. | `.\doki.ps1 up agent`, then `cd <project>`, then `.\doki.ps1 code` (REPL, `--continue` to resume) or `.\doki.ps1 code -p "<task>"` (one-shot; exit code 1 on failure, pipeable stdin, `--output-format json`). `doki help` lists every command. First run auto-builds `control\DokiDex.Cli`. |
...
| **DokiGen Void SwarmUI theme** | On-brand void/cyan/gold SwarmUI skin matching the control panel; installed and set as the DEFAULT SwarmUI theme by `.\setup.ps1 -Media` (only edits an existing `Settings.fds`, never creates one). | Automatic with `-Media`; override per-browser via SwarmUI User Settings → Theme (sets the `sui_theme_id` cookie, which still wins). |
...
| **Z-Image Base** *(optional)* | Non-distilled Z-Image for the quality ceiling — quality default is just more steps. Reuses Turbo's auto-fetched encoder/VAE. Covered transitively by the Z-Image Turbo smoke (shared encoder/VAE/pipeline; Base differs only by more steps) — no dedicated smoke test. | `model='z_image_bf16.safetensors'`, 30–50 steps. `-Models full`. |
| **Chroma1-HD** *(optional)* | Uncensored, FLUX-derived stylized image model. Distinct FLUX-derived `arch=chroma` pipeline — downloaded but no dedicated verify smoke or Doctor row. | `model='Chroma1-HD-fp8mixed-final.safetensors'` (use the `-final` stable variant). `-Models full`. |
| **InstantID face-identity** *(optional, gated)* | Reference-face → identity-preserved SDXL gen via the `cubiq/ComfyUI_InstantID` custom workflow (carries one face's identity onto a fresh image). SDXL, so it **reuses the anime Illustrious/Animagine base** — no new checkpoint; ~4.55 GB add-on weights (IP-Adapter + ControlNet + antelopev2). | Install with `setup.ps1 -FaceId`. Run `doki gen -FaceId -InitImage <face.png> '<prompt>'` (the reference face rides the init-image channel; `-FaceId` requires `-InitImage`) → `comfyuicustomworkflow=InstantID`. ⚠ the runnable `CustomWorkflows\InstantID.json` is the **on-GPU authoring step** (upstream example is a ComfyUI UI-graph, not SwarmUI's API-prompt format); `-FaceId` install wires node+weights until it's authored. |
| **PuLID-Flux face-identity** *(optional, gated)* | Reference-face → identity-preserved **FLUX.1-dev** gen via the `balazik/ComfyUI-PuLID-Flux` custom workflow (the FLUX sibling of the InstantID SDXL row). ⚠ pulls a **NON-GATED ~17GB FLUX fp8 base NOT otherwise on disk** (Kijai `flux1-dev-fp8` unet + `ae`, comfyanonymous `t5xxl`/`clip_l` — DokiDex's bases are Z-Image/SDXL/flux-2-klein, none provide FLUX.1's t5xxl) + `pulid_flux_v0.9.1` (~1.14 GB); **shares InstantID's antelopev2** (not re-downloaded if `-FaceId` ran). | Install with `setup.ps1 -Pulid`. Run `doki gen -Pulid -InitImage <face.png> '<prompt>'` (the reference face rides the init-image channel; `-Pulid` requires `-InitImage`) → `comfyuicustomworkflow=PuLID`. ⚠ the node is **balazik Alpha V0.1.0 / last commit 2024-10-03** (stale — verify it LOADS on the current ComfyUI FIRST), and the runnable `CustomWorkflows\PuLID.json` is the **on-GPU authoring step** (balazik's example is a ComfyUI UI-graph, not SwarmUI's API-prompt format); `-Pulid` install wires node+weights until authored. |
| **InfiniteTalk talking-video** *(optional, gated, heavy)* | Portrait + audio → an audio-driven talking-video (lip-synced) via Kijai's `ComfyUI-WanVideoWrapper` running MeiGen **InfiniteTalk** on the **Wan2.1-I2V-14B** base. ⚠ pulls an **~82GB Wan2.1-I2V-14B base NOT otherwise on disk** (DokiDex ships Wan 2.2, a different arch the adapter can't inject into) + the fp16 InfiniteTalk adapter + chinese-wav2vec2 audio encoder. | Install with `setup.ps1 -InfiniteTalk`. Run `doki gen -InfiniteTalk -InitImage <portrait> -Audio <wav/mp3> '<prompt>'` (requires **both** `-InitImage` and `-Audio`) → `comfyuicustomworkflow=InfiniteTalk`. ⚠ the runnable `CustomWorkflows\InfiniteTalk.json` is the **on-GPU authoring step** (Kijai's example is a UI-graph, MeiGen's are CLI configs), the **32GB fit is unconfirmed** (needs an fp8 base + block-swap; native fp16 14B OOMs), and the audio body-key is pinned on-GPU; `-InfiniteTalk` install wires node+weights until authored. |
| **LatentSync lip-sync re-sync** *(optional, gated, light)* | Audio → lip **re-sync** of an EXISTING clip (edits a video's mouth to the audio) via the `ShmuelRonen/ComfyUI-LatentSyncWrapper` node running **ByteDance LatentSync 1.5** — the **LIGHTER** alternative to InfiniteTalk (~9.8GB / 8GB VRAM vs ~82GB). Self-contained SD-VAE-latent model (zero Wan dependency); pulls the LatentSync-1.5 weights + the **required SD-VAE** (`stabilityai/sd-vae-ft-mse`). **I/O divergence vs InfiniteTalk:** it re-syncs an existing video (the source rides the workflow's video-input channel), so it needs **`-Audio` only** — no portrait, and `-InitImage` is rejected. | Install with `setup.ps1 -LatentSync`. Run `doki gen -LatentSync -Audio <wav/mp3> '<prompt>'` (requires `-Audio`; no `-InitImage`) → `comfyuicustomworkflow=LatentSync`. ⚠ the runnable `CustomWorkflows\LatentSync.json` is the **on-GPU authoring step** (the wrapper's examples are UI-graphs, not SwarmUI API-prompt), and the video-input channel + audio body-key are pinned on-GPU; `-LatentSync` install wires node+weights until authored. |
...
| **Wan 2.2 TI2V-5B** *(quality)* *(optional)* | Higher-quality text-to-video that reliably fits 32 GB; the recommended quality tier (832×480 in ~26 s). fp16 only. | `model='wan2.2_ti2v_5B_fp16.safetensors'`, `textvideoframes=49`, `steps=20`, `cfgscale=3.5`, 832×480, `videofps=24`. `-Models full`. |
...
| **LTXV-2b-0.9.8-distilled** *(fast)* *(optional)* | SwarmUI-native near-real-time video, long clips up to ~257 frames; a speed option below Wan 2.2 (~97 frames 768×512 in ~10.6 s; first-ever gen ~36 s incl. the one-time T5 auto-download). | `model='ltxv-2b-0.9.8-distilled.safetensors'`, `textvideoframes=97`, `steps=8`, `cfgscale=1`, 768×512, `sampler=euler`, `scheduler=normal` (the LTXV 0.9.8 distilled template; without them the arm inherits SwarmUI's uni_pc/simple video defaults). `-Models full`. |
...
| `doki gen "<idea>" [-Video\|-Music\|-Edit\|-I2v\|-Foley\|-FaceId\|-Pulid\|-InfiniteTalk\|-LatentSync]` | Text→media one-liner — picks the SwarmUI recipe, expands the idea via the `:8013` rewriter, POSTs `GenerateText2Image`, opens the result. `-I2v` animates a still (+`-InitImage`), `-Foley` adds synced SFX, `-FaceId` does InstantID (SDXL) face-identity transfer (requires `-InitImage <face.png>`; gated on `setup.ps1 -FaceId` + the on-GPU workflow), `-Pulid` does PuLID-Flux (FLUX.1-dev) face-identity transfer (requires `-InitImage <face.png>`; gated on `setup.ps1 -Pulid` — a ~17GB non-gated FLUX fp8 base + the Alpha/stale balazik node — + the on-GPU workflow), `-InfiniteTalk` does audio-driven talking-video (requires `-InitImage <portrait>` + `-Audio <wav/mp3>`; gated on `setup.ps1 -InfiniteTalk` — an ~82GB Wan2.1-14B base — + the on-GPU workflow), `-LatentSync` does light audio-driven lip **re-sync** of an existing clip (requires `-Audio <wav/mp3>` only; `-InitImage` rejected — the source video rides the workflow's video-input channel; gated on `setup.ps1 -LatentSync` — ~9.8GB ByteDance LatentSync 1.5 — + the on-GPU workflow); plus `-Fast` / `-Upscale` / `-InitImage` / `-Raw` / `-Out`. Needs `up media`. |
| `doki index` | (Re)build the codebase RAG index that backs `code_search` (CPU embed server on `:8090`; never touches the GPU). |
| `doki code ["<task>"]` | Launch the local coding agent in the current directory: REPL (no arg, `--continue` to resume the last session) or one-shot (`-p "<task>"` / a bare task string — exit code 1 on failure, `--output-format json` available). Needs `up agent`. Auto-builds `DokiDex.Cli` on first run. |
| `doki help` | Print every `doki` command on one screen — sourced from the actual command handling, plus the `gen` flag/modifier note and the Crush power-user pointer. Bare `doki` still defaults to `status`. |
...
A single-process **WPF cockpit on .NET 9** (premium void/cyan/gold theme; native splash + a cinematic boot sequence) — a reactive face that polls status **in-process (native C#)** and shells `doki.ps1` only for lifecycle actions, never re-implementing control logic. Launch via `control.bat` (double-click; first run builds Release **and** creates a console-free `DokiDex.lnk` launcher with the arc-reactor icon), the `DokiDex.lnk` shortcut thereafter, or `doki panel`.

| Area | What it does |
|---|---|
| **Shell & polling** | Repo-root auto-discovery; ~2-second native in-process (C#) status poll via a native StatusProbe (skips overlapping ticks; "doki status unavailable" on null); three-zone shell (left rail / content / status strip) with Dashboard ↔ Logs nav. |
...
| **Arc-reactor icon + `DokiDex.lnk`** | First-run `control.bat` / `doki panel` builds the WinExe (whose embedded icon + native splash come from the committed `assets/dokidex.ico` and `assets/splash.png`, authored offline by `make-icon.ps1` / `make-splash.ps1`) and creates a console-free `DokiDex.lnk` shortcut (`make-shortcut.ps1`) carrying the arc-reactor icon — the day-to-day entry point. |
| **Theme & tests** | Premium void/cyan/gold theme (`Themes/Palette.xaml`) + value converters; **124 xUnit tests** across **14 classes** — covering status parsing, GPU view-model, service view-model, log classification, and the updater (`dotnet test control\DokiDex.Control.Tests`). |
...
| Feature | What it does | How to use |
|---|---|---|
| **`doki verify`** — full-stack smoke test | Cycles GPU modes (agent → coexist → media), runs live capability smokes with a real API call each, prints a PASS/SKIP/FAIL table, restores `agent` (the default resting state), and exits 0 only if zero failures. **17 result rows: 5 always run, 12 SKIP cleanly when a full-tier model (or coder-big) is absent.** | `.\doki.ps1 verify` |
...
| **`doki test`** — unit tests | Runs the fast no-GPU suite: installer-helper + `status json`-contract PowerShell tests (AST-extracted from the real scripts), the sqlite/FTS5 memory tests, and the control-panel xUnit incl. the auto-updater (160 assertions total). | `.\doki.ps1 test` |
| **Releases & auto-update** | Push a `v*` tag (`git tag vX.Y.Z && git push origin vX.Y.Z`) → `.github/workflows/release.yml` builds a self-contained single-file `DokiDex-vX.Y.Z-win-x64.exe`, embeds the version, and publishes it as a GitHub release — the payload the panel's in-app updater downloads and swaps in place. `workflow_dispatch` builds/validates only. The exe is a **standalone**, self-contained app: on first run it installs its own home via a Setup Wizard (or adopts an existing folder) and runs independent of a cloned repo. | `git tag v* && git push --tags` |
...
| `.\setup.ps1 -Media` | The uncensored image+video stack, fully headless: .NET 9 SDK (from core setup) + git, clone+build SwarmUI, MagicPrompt extension, DokiGen Void theme, ComfyUI backend via the `InstallConfirmWS` WebSocket (no GUI wizard), **lean** models (Z-Image Turbo, Wan 2.1 1.3B). |
