...
| **`doki code` local coding agent** | Terminal coding agent that mirrors Claude Code: a ReAct loop over **Read** (line-windowed) / **Grep** (regex + optional path/glob) / **Edit** (SEARCH/REPLACE blocks, whitespace-flexible fuzzy applier, miss → nearby-line hint) / **Write** / **Bash** (PowerShell). Read and Grep run freely; Edit/Write/Bash are gated behind a per-action `[y]es / [a]lways / [n]o` approval (default: no) with a colored diff or command preview. In-session `/undo` stack; edits are plain working-tree changes (review with `git diff`). Up to 24 tool hops per turn. Default model: `coder-fast`. Slash commands: `/model` `/undo` `/clear` `/cwd` `/exit`. | `.\doki.ps1 up agent`, then `cd <project>`, then `.\doki.ps1 code` (REPL) or `.\doki.ps1 code "<task>"` (one-shot). First run auto-builds `control\DokiDex.Cli`. |
...
| `doki code ["<task>"]` | Launch the local coding agent in the current directory: REPL (no arg) or one-shot (arg = task string). Needs `up agent`. Auto-builds `DokiDex.Cli` on first run. |
