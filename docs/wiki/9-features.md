...
| `doki gen "<idea>" [-Video\|-Music\|-Edit\|-I2v\|-Foley\|-FaceId]` | Text→media one-liner — picks the SwarmUI recipe, expands the idea via the `:8013` rewriter, POSTs `GenerateText2Image`, opens the result. `-I2v` animates a still (+`-InitImage`), `-Foley` adds synced SFX, `-FaceId` does InstantID face-identity transfer (requires `-InitImage <face.png>`; gated on `setup.ps1 -FaceId` + the on-GPU workflow); plus `-Fast` / `-Upscale` / `-InitImage` / `-Raw` / `-Out`. Needs `up media`. |
