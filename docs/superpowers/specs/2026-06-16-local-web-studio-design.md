...
- **P5 — Edit canvas** (mask inpaint) + a broad slice of **top backlog features** (29 this round; control **235/0**, doki-gen **84/0**), each with a pure unit-tested core or reuse of a tested path, + graceful "start agent/media mode" degradation:
...
- **The 3D-blockout three.js scene editor** — needs three.js bundled offline (≈600 KB, no source to embed in the single-file SPA) or a from-scratch WebGL editor, plus eyes-on-a-display verification of 3D output. Its depth-map → Depth-ControlNet *output* is the shipped ControlNet (+ control-image upload / sketch). (node-flow's graph IS now built — node-lite Flow.)
