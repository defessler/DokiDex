...
- **P5 — Edit canvas** (mask inpaint) + a broad slice of **top backlog features** (28 this round; control **230/0**, doki-gen **84/0**), each with a pure unit-tested core or reuse of a tested path, + graceful "start agent/media mode" degradation:
...
- **Two interactive editor *canvases* whose value IS the canvas, with no testable core — and whose buildable slices already shipped:** the **node-flow graph** (linear slice = the shipped CSV batch + **saved recipes**) and the **3D-blockout** three.js scene editor (output = a depth map → the shipped ControlNet). Only the visual editor canvas is unbuilt, and it can't be verified headless.
