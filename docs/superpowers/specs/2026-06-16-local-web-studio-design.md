...
- **P5 — Edit canvas** (mask inpaint) + a broad slice of **top backlog features** (27 this round; control **216/0**, doki-gen **84/0**), each with a pure unit-tested core or reuse of a tested path, + graceful "start agent/media mode" degradation:
...
**What's genuinely left** (tracked as task #27 / P6) — after building every SwarmUI-param feature against the source schema, every pure-logic feature, and every reuse-of-a-tested-path SPA feature (including ControlNet stacking), the remainder has NO buildable-and-verifiable slice left here; each is blocked by a physical artifact:
- **A specific absent model whose output IS the feature:** SAM/CLIPSeg *click-mask* segmenter (the prompt-`<segment:>` slice shipped), the FaceID-specific model (the general IP-Adapter mechanism shipped), the SUPIR *pipeline* (any SUPIR upscale model is already reachable via `-Upscaler`).
- **A sidecar tool with no install path in this repo:** **Demucs** stems, kohya **LoRA training**. (RIFE is moot — frame interpolation shipped as the SwarmUI param.)
- **Two big interactive editors whose value IS the canvas, with no testable core:** the **node-flow graph** (its linear slice = the shipped CSV batch) and the **3D-blockout** three.js scene editor (its output = a depth map → the shipped ControlNet). Only the *editor canvas* is unbuilt, and it can't be verified headless.
