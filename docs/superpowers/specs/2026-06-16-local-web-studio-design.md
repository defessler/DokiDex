...
- **P5 — Edit canvas** (mask inpaint) + a broad slice of **top backlog features**, each with a pure unit-tested core + graceful "start agent/media mode" degradation:
  - *Prompt composition:* aspect-ratio presets; **LoRA mixer** (multi-LoRA + weights → `<lora:>` tags); user **negative-prompt** knob; **style chips** (stackable aesthetic bundles → +/- fragments); dynamic-prompt **wildcards** (`__name__` per-seed, resolved prompt recorded); **steerable rewriter** (user-directed, conversational iterate); **upscale content-class engine** selector (`-Upscaler` → refiner upscale model).
  ...
  - *Orchestration:* **script-to-shotlist Director** (local LLM → editable shots → batch gen); **CSV batch** generation; **Exploration Mode** (diverge one prompt into N seed-varied drafts).
