...
- **P5 — Edit canvas** (mask inpaint) + a broad slice of **top backlog features**, each with a pure unit-tested core + graceful "start agent/media mode" degradation:
  - *Prompt composition:* aspect-ratio presets; **LoRA mixer** (multi-LoRA + weights → `<lora:>` tags); user **negative-prompt** knob; **style chips** (stackable aesthetic bundles → +/- fragments); dynamic-prompt **wildcards** (`__name__` per-seed, resolved prompt recorded); **steerable rewriter** (user-directed, conversational iterate).
  ...
  - *Orchestration:* **script-to-shotlist Director** (local LLM → editable shots → batch gen); **CSV batch** generation (header row → one gen per row).
...
**Remaining backlog = pure model-stubs or GPU-verify front-ends** (no substantive buildable+verifiable layer in a headless dev env — tracked as task #27 / P6, not built): features that need a model installed to do anything (per-ref ControlNet stacking + preprocessors, character-reference InstantID/PuLID/FaceID, Smart-Layers SAM/CLIPSeg segmenter, SUPIR/upscaler engines, **Demucs** stem separation), the **3D-blockout Scene-to-Image** (Depth-ControlNet) and **layout drag-canvas** (a near-duplicate of Cast for the regional mechanism), video keyframe/extend (need the FLF2V/motion nodes verified on the GPU), and big live front-ends (realtime scratchpad, sketch-live, node-flow). For these the only "buildable" layer is a stub that always reports "not installed" — shipped already as the graceful-degradation pattern where it adds value (voice registry, LoRA list). See the platform backlog for the full list + per-item local-feasibility notes.
