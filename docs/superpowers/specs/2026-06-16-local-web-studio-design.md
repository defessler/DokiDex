...
- **P5 — Edit canvas** (mask inpaint + a **magic-wand** color-select + **SAM** semantic click for click→mask + **background removal** → transparent PNG + **directional outpaint** over the edit path) + the **full top-backlog set** (55 this round; control **340/0**, doki-gen **94/0**), each with a pure unit-tested core or reuse of a tested path, + graceful degradation:
  ...
  - *Video (gated on ffmpeg, found on PATH or bundled):* **extract a keyframe** (last/first frame of a clip → an i2v/FLF2V keyframe) + **join clips** (filter_complex concat) + **one-click auto clip-extend** (chains extract→i2v→join with client-side job polling) — the clip-extend / storyboard-strip primitives *and* their convenience macro (`Ffmpeg.*Args` pure-tested; degrade cleanly when ffmpeg is absent, like the SAM/Demucs sidecars).
  - *Edit canvas extras:* **directional outpaint** (padded init + border mask over the edit path); **richer mask toolkit** (invert + adjustable brush, scroll-to-resize).
...
1. **UX polish over already-shipped capability** — consolidating inpaint+outpaint+sketch behind one "edit strength" dial; the last mask extras (load-previous-mask / dedicated eraser — invert + brush-size now ship); drag-the-border *handles* (directional outpaint already ships); per-region reference drop; a resizable generation window; a stacked edit-history strip. Incremental refinements, not missing features (the auto clip-extend macro now ships).
