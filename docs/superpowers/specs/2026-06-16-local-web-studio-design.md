...
- **P5 — Edit canvas** (mask inpaint) + a broad slice of **top backlog features** (20 this round; control **211/0**, doki-gen **74/0**), each with a pure unit-tested core + graceful "start agent/media mode" degradation:
  ...
  - *Composers:* structured **music composer** (lyrics/duration/BPM); **multi-character "Cast"** (base + isolated per-character regions → `<object:>`); **camera** control compilation for video (preset + signed axes → tokens).
...
**Remaining backlog can't be built CORRECTLY headless** (tracked as task #27 / P6) — and building it blind would ship code that *looks* done but is silently broken, which is worse than honest deferral:
- **Unverifiable SwarmUI body-param names** → silent no-op risk. ControlNet stacking, FLF2V start/end keyframes, seamless-tile all need GenerateText2Image keys that are NOT in this repo (only `<segment:>`/`<lora:>`/`initimage`/`refiner*`/`video*` are confirmed). A wrong guessed key is silently ignored by SwarmUI — it would pass a unit test and do nothing live. These need the SwarmUI param schema (`ListT2IParams` on a running instance) to build right.
- **Need a model/tool installed to do anything** (the only buildable layer is a "not installed" stub, already shipped as graceful degradation where it helps): FaceID/InstantID/PuLID, SAM/CLIPSeg Smart-Layers segmenter, SUPIR upscaler, **Demucs** stems, RIFE interpolation, in-app LoRA training.
- **Big live front-ends that need the GPU loop to mean anything:** realtime scratchpad, sketch-live, node-flow canvas, 3D-blockout. (The layout drag-canvas is a near-duplicate of the shipped Cast regional mechanism.)
