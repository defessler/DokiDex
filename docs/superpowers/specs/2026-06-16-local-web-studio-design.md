...
- **P5 — Edit canvas** (mask inpaint + a **magic-wand** color-select for click→mask) + a broad slice of **top backlog features** (33 this round; control **245/0**, doki-gen **86/0**), each with a pure unit-tested core or reuse of a tested path, + graceful "start agent/media mode" degradation:
...
**What's genuinely left — exactly two, and they are irreducible** (tracked as task #27 / P6). SUPIR + InstantID are reachable (custom-workflow runner), Demucs is built, node-flow's graph is built (node-lite Flow), the SAM *click→mask UX* ships model-free (magic-wand + prompt-`<segment:>`), and 3D-blockout is built (server-side software depth rasterizer). The final two are **pure machine-learning-model execution — there is no algorithm to write, only a model to run/train**, and the model/trainer + GPU don't exist in this environment:
- **SAM *semantic* selection** — knowing *which object* a click is on IS the learned SAM model; no classical code reproduces it (the model-free click→mask UX is shipped).
- **kohya LoRA training** — training a LoRA IS gradient descent on the GPU via the trainer (and sd-scripts' support for the Z-Image base arch is unconfirmed); no code substitutes for the training run.
Building either here would be fabrication, not completion. Each unblocks the instant its weights/trainer exist on the GPU box.
