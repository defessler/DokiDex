...
- **P5 — Edit canvas** (mask inpaint + a **magic-wand** color-select for click→mask) + a broad slice of **top backlog features** (32 this round; control **240/0**, doki-gen **86/0**), each with a pure unit-tested core or reuse of a tested path, + graceful "start agent/media mode" degradation:
  ...
  - *Audio/voice:* local **TTS voice** surface (Chatterbox `:8004`, file-based voice registry) + **pronunciation dictionary** (alias substitution); **Demucs stem separation** (standalone sidecar, repo's venv pattern, gated on `setup.ps1 -Demucs`).
...
**What's genuinely left — exactly three** (tracked as task #27 / P6). SUPIR + InstantID are now reachable (the custom-workflow runner), Demucs is built (sidecar, gated on `setup.ps1 -Demucs`), and node-flow's graph is built (node-lite Flow). The final three each need a physical artifact this environment lacks, and building blind would only fake them:
- **SAM click-mask (semantic)** — needs the SAM model; *semantic* mask-from-a-click IS the model. The click→mask UX is now shipped two model-free ways (the **magic-wand** color-select + prompt-`<segment:>`); only the SAM-grade *semantic* selection needs the weights.
- **kohya LoRA training** — needs kohya/sd-scripts + GPU; its CLI + dataset-folder structure are complex enough that a blind launcher would ship likely-broken (unlike the simple, shipped Demucs CLI). Build it where kohya is installed.
- **3D-blockout** — needs three.js bundled offline (≈600 KB, no source to embed in the single-file SPA) or a from-scratch WebGL editor, plus eyes-on verification of 3D output. Its depth-map → Depth-ControlNet output is the shipped ControlNet (+ control-image upload / sketch).
