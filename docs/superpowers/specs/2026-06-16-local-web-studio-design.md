...
- **P5 — Edit canvas** (mask inpaint) + a broad slice of **top backlog features** (22 this round; control **213/0**, doki-gen **81/0**), each with a pure unit-tested core + graceful "start agent/media mode" degradation:
  ...
  - *Composers:* structured **music composer** (lyrics/duration/BPM); **multi-character "Cast"** (base + isolated per-character regions → `<object:>`); **camera** control compilation for video (preset + signed axes → tokens); **ControlNet** structure conditioning and **FLF2V end-keyframe** — both built against SwarmUI's *source-extracted* param schema (`controlnet*` / `videoendimage` via the real `CleanTypeName`), gated on the respective model being installed.
...
**Remaining backlog needs a SPECIFIC absent model/tool to produce output** (tracked as task #27 / P6) — for these the blocker is the model/tool itself, not param knowledge, so no headless code is verifiable:
- **Need the model installed:** FaceID/InstantID/PuLID (+ uncertain whether the Z-Image base has an IP-Adapter), SAM/CLIPSeg click-mask segmenter (the *prompt*-driven `<segment:>` slice already shipped), SUPIR upscaler pipeline.
- **Need the sidecar tool installed:** **Demucs** stems, RIFE interpolation, kohya **LoRA training**.
