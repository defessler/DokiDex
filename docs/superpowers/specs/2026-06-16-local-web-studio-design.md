...
- **P5 — Edit canvas** (mask inpaint + a **magic-wand** color-select + **SAM** semantic click for click→mask) + the **full top-backlog set** (35 this round; control **250/0**, doki-gen **86/0**), each with a pure unit-tested core or reuse of a tested path, + graceful degradation:
