...
- **P5 — Edit canvas** (mask inpaint + a **magic-wand** color-select + **SAM** semantic click for click→mask + **background removal** → transparent PNG) + the **full top-backlog set** (52 this round; control **336/0**, doki-gen **94/0**), each with a pure unit-tested core or reuse of a tested path, + graceful degradation:
...
What remains is **not** a clean build — it is one of four kinds, and each is recorded as such rather than explained away:
1. **Heavy bespoke editors** — a unified mixed-mode edit canvas (inpaint+outpaint+sketch with frame handles / drag-border outpaint / rich mask toolkit / per-region ref / resizable gen window / stacked history), a **keyframe storyboard strip + clip-extend** (needs an ffmpeg concat sidecar + frame extraction), and **warp markers** (a waveform editor + a phase-vocoder DSP sidecar). These are real but each is a large, multi-part editor — a separate build effort, not a single tested slice.
2. **Fuzzy/risky dials** — Stylize/Weird/Variety map to **CFG**, which is model-dependent (Turbo needs cfg≈1; overriding it breaks the shipped fast path); the backlog's own "never expose raw CFG" hedge holds. Not shipped on merit, not on inability.
3. **Upstream-gated** — workflow→form *field injection*, ACE-Step audio2audio/repaint, S2V/lip-sync, VACE video-edit, relight: each needs a SwarmUI/ComfyUI capability not present on this install; the backlog marks them "re-qualify on X." The gated-integration pattern already covers every row where the capability *does* exist.
4. **Trivia redundant with shipped controls** — a Draft/Final segmented toggle (the Fast MOD already is this), named-slider relabels, denoise-zone labels, pin-tools.

The substantial, buildable, verifiable backlog is exhausted; what is left is large editor subsystems, a model-risky dial, upstream-blocked rows, or relabels of controls that already ship.
