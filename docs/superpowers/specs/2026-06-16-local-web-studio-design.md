...
- **P5 — Edit canvas** (mask inpaint) + a broad slice of **top backlog features** (26 this round; control **215/0**, doki-gen **85/0**), each with a pure unit-tested core or reuse of a tested path, + graceful "start agent/media mode" degradation:
  - *Prompt composition:* aspect-ratio presets; **LoRA mixer** (multi-LoRA + weights → `<lora:>` tags); user **negative-prompt** knob; **style chips** (stackable aesthetic bundles → +/- fragments); dynamic-prompt **wildcards** (`__name__` per-seed, resolved prompt recorded); **steerable rewriter** (user-directed, conversational iterate); **upscale content-class engine** selector (`-Upscaler`); **promptable segmentation** (`-Segment` → `<segment:any-keyword>`, generalizing `-Face`).
  - *Composers:* structured **music composer** (lyrics/duration/BPM); **multi-character "Cast"** (base + isolated per-character regions → `<object:>`); **camera** control compilation for video; **image reference** (init image → IP-Adapter `useipadapterforrevision`/`ipadapterweight`); **sketch-to-image** (drawing canvas → ControlNet control image).
  - *SwarmUI features built against the SOURCE-EXTRACTED param schema* (via the real `CleanTypeName` = lowercase-letters-only), each gated on the relevant model/node being installed: **ControlNet** (`controlnetmodel/strength/imageinput/preprocessor`), **FLF2V end-keyframe** (`videoendimage`), **frame interpolation** (`videoframeinterpolationmethod/multiplier`).
  - *Realtime:* **scratchpad** (debounced turbo re-render as you type, over the tested Fast path).
  - *Orchestration:* **script-to-shotlist Director** (local LLM → editable shots → batch gen); **CSV batch** generation; **Exploration Mode** (diverge one prompt into N seed-varied drafts); **per-card refine-from-result** (vary / face-fix / hi-res / upscale — re-run the card image as img2img).
  - *Audio/voice:* local **TTS voice** surface (Chatterbox `:8004`, file-based voice registry) + **pronunciation dictionary** (alias substitution).
  - *Library:* **search + kind filter** over the sidecar index.
  - Shared `LocalLlm` helper backs the Director + rewriter; all LLM/TTS calls degrade cleanly when the backend is down (verified to the same bar as the gen path).
...
**What's genuinely left** (tracked as task #27 / P6) — after building every SwarmUI-param feature against the source schema, every pure-logic feature, and every reuse-of-a-tested-path SPA feature, the remainder is only:
- **A specific absent model whose output IS the feature:** SAM/CLIPSeg *click-mask* segmenter (the prompt-`<segment:>` slice shipped), FaceID-specific model (the general IP-Adapter mechanism shipped), the SUPIR *pipeline* (any SUPIR upscale model is already reachable via `-Upscaler`).
- **A sidecar tool with no install path in this repo:** **Demucs** stems, kohya **LoRA training** (standalone RIFE is moot — frame interpolation shipped as the SwarmUI param).
- **Two big interactive editors whose value IS the canvas, with no headless-verifiable core:** the **node-flow graph** canvas (the linear-recipe slice could ship first) and the **3D-blockout** three.js scene editor.
- **ControlNet N-unit stacking** — an extension of the shipped single-unit ControlNet (keys known: `controlnettwo*`).
