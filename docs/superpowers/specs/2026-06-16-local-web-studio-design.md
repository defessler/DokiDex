...
- **P5 — Edit canvas** (mask inpaint) + a broad slice of **top backlog features** (31 this round; control **240/0**, doki-gen **86/0**), each with a pure unit-tested core or reuse of a tested path, + graceful "start agent/media mode" degradation:
...
- **SAM click-mask** — needs the SAM model; *semantic* mask-from-a-click IS the model (a color-flood "magic wand" would be a different, lesser tool, not SAM). Prompt-`<segment:>` slice shipped.
